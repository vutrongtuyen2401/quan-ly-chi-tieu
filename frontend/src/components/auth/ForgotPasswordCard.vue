<template>
  <div class="auth-card-stitch">
    ...
    <div class="step-indicator-row">
      <span class="step-tag">Bước 1 / 2: Xác minh danh tính</span>
    </div>

    <!-- Header Title & Subtitle -->
    <div class="card-header-block">
      <h2 class="card-title">Quên mật khẩu?</h2>
      <p class="card-subtitle">Nhập thông tin tài khoản để tiếp tục.</p>
    </div>
    ...
    <form class="auth-form-body" @submit.prevent="handleSubmit">
      ...
      <div class="form-field-group">
        <div class="field-label-row">
          <label class="field-label" for="forgot-soullamp">Bản Mệnh Hồn Đăng</label>
          <span class="soul-lamp-badge">Chuỗi bí mật hộ mệnh</span>
        </div>
        <div class="field-input-wrap">
          <span class="material-symbols-outlined input-icon-left" aria-hidden="true">vpn_key</span>
          <input
            id="forgot-soullamp"
            v-model="form.soul_lamp"
            :type="showSoulLamp ? 'text' : 'password'"
            class="stitch-auth-input with-left-icon with-toggle"
            placeholder="Nhập Bản Mệnh Hồn Đăng"
            required
            :disabled="loading"
          ...
          <button
            ...
            :aria-label="showSoulLamp ? 'Ẩn Hồn Đăng' : 'Hiện Hồn Đăng'"
            @click="showSoulLamp = !showSoulLamp"
          >
            <span class="material-symbols-outlined toggle-icon">
              {{ showSoulLamp ? 'visibility_off' : 'visibility' }}
            </span>
          </button>
        </div>
      </div>
    ...
    <div class="security-notice-strip">
      <span class="material-symbols-outlined notice-icon">lock_clock</span>
      <span>Giới hạn 5 lần thử / 15 phút để bảo vệ tài khoản.</span>
    </div>
  </div>
</template>
...
.soul-lamp-badge {
  ...
  border: 1px solid rgba(43, 138, 130, 0.25);
  padding: 2px 8px;
  border-radius: 6px;
}
